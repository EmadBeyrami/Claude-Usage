...
final class TranscriptScannerTests: XCTestCase {
    ...
    func testProjectComesFromCwdNotTheDirectorySlug() async throws {
        // The directory under a profile's projects folder flattens both
        // separators and underscores to "-", so it can't be reversed into a
        // real name.
        try write("C--Users-Alex-js-projects-my-app/s.jsonl", [
            line("r1", at: Date(), output: 100,
                 cwd: #"C:\\Users\\Alex\\js_projects\\my_app"#),
        ])
        let stats = await scanner().scan(force: true)
