...
// it's a quick way to check numbers by hand.
//
// The CLI is a plain Terminal process, not sandboxed, so unlike the menu bar
// app it can look for ~/.claude on its own rather than needing a folder
...
let discovered = ProfileStore.discover(grantedFolders: ProfileStore.autoDiscoverCandidates())
