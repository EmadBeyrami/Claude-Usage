...
final class ProfileDiscoveryTests: XCTestCase {
    ...
    /// `autoDiscoverCandidates` stands in for the sandboxed app's granted-folder
    /// list here — it's the same blind-filesystem search the CLI uses, and it
    /// exercises `discover(grantedFolders:)` exactly the way real callers do.
    private func discover(env: [String: String] = [:], extra: [String] = []) -> [Profile] {
        let candidates = ProfileStore.autoDiscoverCandidates(environment: env, home: home)
            + extra.map { URL(fileURLWithPath: $0, isDirectory: true) }
        return ProfileStore.discover(grantedFolders: candidates)
    }
...
final class ProfileCredentialsTests: XCTestCase {
    ...
    func testFallsBackToNoTokenWhenNothingIsStored() {
        // No credentials file, and nothing pasted into Settings for this
        // profile: there is nowhere left to look, sandboxed or not.
...
final class ManualTokenTests: XCTestCase {

    private func profile() -> Profile {
        Profile(configDir: FileManager.default.temporaryDirectory
            .appendingPathComponent("manual-\(UUID().uuidString)", isDirectory: true), isDefault: false)
    }

    func testSaveThenReadRoundTrips() {
        let p = profile()
        defer { CredentialStore.clearManualToken(for: p) }

        CredentialStore.saveManualToken("plain-token-value", for: p)
        XCTAssertTrue(CredentialStore.hasManualToken(for: p))
        XCTAssertEqual(CredentialStore.read(for: p).token, "plain-token-value")
    }

    func testClearRemovesIt() {
        let p = profile()
        CredentialStore.saveManualToken("x", for: p)
        CredentialStore.clearManualToken(for: p)
        XCTAssertFalse(CredentialStore.hasManualToken(for: p))
    }

    func testAcceptsAFullCredentialsJsonBlobPastedFromKeychainAccess() {
        let p = profile()
        defer { CredentialStore.clearManualToken(for: p) }

        let future = (Date().timeIntervalSince1970 + 3600) * 1000
        let blob = #"{"claudeAiOauth":{"accessToken":"tok-from-json","expiresAt":\#(future)}}"#
        CredentialStore.saveManualToken(blob, for: p)

        let creds = CredentialStore.read(for: p)
        XCTAssertEqual(creds.token, "tok-from-json")
        XCTAssertFalse(creds.expired)
    }

    func testFlagsAnExpiredPastedToken() {
        let p = profile()
        defer { CredentialStore.clearManualToken(for: p) }

        let past = (Date().timeIntervalSince1970 - 3600) * 1000
        let blob = #"{"accessToken":"stale","expiresAt":\#(past)}"#
        CredentialStore.saveManualToken(blob, for: p)

        XCTAssertTrue(CredentialStore.read(for: p).expired)
    }
}
...
final class ProfileHistoryTests: XCTestCase {
    ...
    func testSlugIsReadable() {
        XCTAssertEqual(History.slug("/Users/alex/.claude-work"), "users-alex-claude-work")
    }
}
