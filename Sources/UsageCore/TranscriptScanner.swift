...
public struct LogStats: Codable, Sendable, Equatable {
    ...
    /// False when the profile's projects directory is missing or unreadable.
...
struct LogEntry {
    ...
    let tokens: Int
    let cost: Double
    /// From the entry's `cwd`. The directory name under a profile's projects
    /// folder is a slug with separators and underscores both flattened to "-",
    /// so it can't be reversed into a real name; cwd is exact.
    let project: String?
}

/// Walks one profile's projects directory and totals token usage and cost.
///
/// Re-reading every recent transcript on each poll is fine for a one-shot CLI
/// and not fine for a process that polls every 60s — an active account can have
/// gigabytes of transcripts with single files tens of megabytes each. So each
/// file's parsed entries are cached and only bytes appended since the last read
/// are parsed.
public actor TranscriptScanner {
    ...
    public init(projectsDirectory: URL, history: History? = nil) {
